...
@get = (path, handler) -> getMap[path] = handler; return
...
@listen = (port, host) ->
    ...
    sys.puts 'Server at http://' + (host or '127.0.0.1') + ":#{port}/"
    return
...
extname = (path) ->
    ...
    return (if index < 0 then '' else path.substring index)
        ...
        readFile filename, (err, data) ->
            if err
                ...
                headers =
                    ...
                    'Content-Length' : body.length
                headers['Cache-Control'] = 'public' unless DEBUG
